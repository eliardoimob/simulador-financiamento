...
function getSubsidioMCMV(renda, valorImovel, categoria, temFgts, temCo) {
  ...
  if (renda <= 2850) {
    ...
    if (renda >= p[2].r) subsidio = 0;
      ...
      const seg = renda <= p[1].r ? [p[0], p[1]] : [p[1], p[2]];
      const m = (seg[1].s - seg[0].s) / (seg[1].r - seg[0].r);
...
function solveMax(V, R, prazoAnos, taxaAnual, ltv, sistema, idade) {
  ...
  const DFI = 0.000138;
  const MIP0 = 0.000038;
  const mip = MIP0 * (1 + (idade - 20) * 0.05);
  ...
  for (let k = 0; k < 50; k++) {
    const mid = (lo + hi) / 2;
    const varEnc = mid * mip;
    const enc = fixo + varEnc;

    const base = sistema === 'SAC' ? (mid / n) + (mid * i) : pricePMT(mid, i, n);
    if (base + enc <= limite) {
      ok = mid;
      p1 = base + enc;
      pf = sistema === 'SAC' ? (mid / n) + (mid / n * i) + enc : p1;
      lo = mid;
    } else {
      hi = mid;
    }
