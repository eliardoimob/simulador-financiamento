// functions/simular/index.js (V51.0 - Sem Captcha)
...
function solveMax(V, R, prazoAnos, taxaAnual, ltv, sistema, idade) { const n = prazoAnos * 12, i = (taxaAnual / 100) / 12, limite = R * 0.30; const TAXA_ADMIN = 25, DFI = 0.000138, MIP0 = 0.000038, mip = MIP0 * (1 + (idade - 20) * 0.05); const fixo = TAXA_ADMIN + V * DFI; let lo = 0, hi = V * ltv, ok = 0, p1 = 0, pf = 0; for (let k = 0; k < 50; k++) { const mid = (lo + hi) / 2, varEnc = mid * mip, enc = fixo + varEnc; const base = sistema === 'SAC' ? (mid / n) + (mid * i) : pricePMT(mid, i, n); if (base + enc <= limite) { ok = mid; p1 = base + enc; pf = sistema === 'SAC' ? (mid / n) + (mid / n * i) + enc : p1; lo = mid; } else hi = mid; } return { fv: ok, p1, pf }; }
function computeSimulation(payload) { const { valorImovel: V, rendaMensal: R, categoria: cat, idadeAnos: idade, flags } = payload; const { fgts: temFgts, subsidioRecebido: recebeuSub, co: temCo } = flags; if (!V || !R || !cat) throw new Error("Dados insuficientes"); const faixa = (recebeuSub && V <= 1500000) ? { ...REGRAS.SBPE, programa: 'SBPE' } : resolverFaixa(R, V, cat); if (!faixa) { return { erroRegra: "Valor acima do teto" }; } const prazoAnos = faixa.prazoMaxAnos || Math.max(1, Math.min(35, 80 - idade)); const taxa = typeof faixa.taxa === 'function' ? faixa.taxa(R) : faixa.taxa; const subsidio = (faixa.programa === 'MCMV' && (faixa.id === 'Faixa 1' || faixa.id === 'Faixa 2')) ? getSubsidioMCMV(R, V, cat, temFgts, temCo) : 0; let ltvSac, ltvPrice; if (faixa.programa === 'MCMV') { ltvSac = ltvPrice = typeof faixa.ltv === 'object' ? faixa.ltv[cat] : faixa.ltv; } else if (faixa.programa === 'SBPE') { ltvSac = faixa.ltv.sac; ltvPrice = faixa.ltv.price; } else { ltvSac = ltvPrice = faixa.ltv; } const sac = solveMax(V, R, prazoAnos, taxa, ltvSac, 'SAC', idade); const price = solveMax(V, R, prazoAnos, taxa, ltvPrice, 'PRICE', idade); const entradaSac = Math.max(0, V - sac.fv - subsidio); const entradaPrice = Math.max(0, V - price.fv - subsidio); const custosDoc = V * 0.045; return { linha: `${faixa.programa || 'SFI'} — ${faixa.id}`, prazoMeses: prazoAnos * 12, taxaAnual: taxa, subsidio, sac: { ltv: ltvSac, entrada: entradaSac, financiamento: sac.fv, p1: sac.p1, pf: sac.pf }, price: { ltv: ltvPrice, entrada: entradaPrice, financiamento: price.fv, p1: price.p1 }, custosDoc }; }

// --- INTEGRAÇÕES (Trello, CORS) ---
const BRL = (n) => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(n) || 0); const pct = (n) => `${(Number(n) || 0).toFixed(2).replace('.', ',')}%`; const safe = (s) => (s ?? '').toString().trim();
...
export async function onRequestPost(context) {
    ...
    if (!allowedOrigins.includes(origin) && !origin.endsWith('.pages.dev')) { // Permite previews da Cloudflare
        return new Response(JSON.stringify({ error: 'Origin not allowed' }), { status: 403, headers: corsHeaders(origin) });
    }
    
    try {
        const body = await request.json();
        // Captcha não é mais lido do body
        const { contato = {}, origem = '', utm = null, ...simulationPayload } = body;
        
        // A verificação do Captcha foi removida
...
export async function onRequestOptions(context) {
    ...
    return new Response(null, { status: 204, headers: { 'Access-Control-Allow-Origin': origin, 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type', 'Allow': 'POST, OPTIONS' } });
}
